...
if (empty($location_id)) {
	...
	$location_data = $flag.'<span class="edost_city_name">'.$city.$s.'</span><span class="edost_city_link" onclick="edost_catalogdelivery.window(\'getcity\')">'.$catalogdelivery_sign['change'].'</span>';
...
if ($mode != 'manual' && $mode != '') {
	...
	if (!empty($format['data'])) foreach ($format['data'] as $f_key => $f) if (!empty($f['tariff']))
		foreach ($f['tariff'] as $k => $v) if (isset($v['id'])) {
			if (!empty($format['show_ico'])) {
				...
				if ($ico !== false) $s .= '<img class="edost_ico edost_ico_'.($minimize ? 'small' : 'normal2').'" src="'.$ico.'" border="0">';
				else $s .= '<div class="edost_ico'.($minimize ? '_small' : '').'"></div>';
			}
			...
			if ($s2 != '') $s .= ' <span class="edost_format_name">('.$s2.')</span>';

			$s .= '<span class="edost_format_name"> - </span>';
			if (isset($v['free'])) $s .= '<span class="edost_format_price edost_price_free">'.$v['free'].'</span>';
			else $s .= '<span class="edost_format_price edost_price">'.(isset($v['pricetotal_formatted']) ? $v['pricetotal_formatted'] : $v['price_formatted']).'</span>';
	...
	else $s = '<div class="edost edost_catalogdelivery_inside">'.$s.'</div>';
