...
session_start();
if(!empty($_GET['currency'])){$_SESSION["newcurrency"]=$_GET['currency'];};
...
<head>
	...
	<meta name="yandex-verification" content="c4b2629273de3740"/>
	...
	<?//раскладываем showhead на составляющие.start?>
	...
	<?//раскладываем showhead на составляющие.end?>

	<?$APPLICATION->AddHeadString('<script>BX.message('.CUtil::PhpToJSObject( $MESS, false ).')</script>', true);?>
	<?if($bIncludedModule)
		CNext::Start(SITE_ID);?>
	<script async src="https://www.googletagmanager.com/gtag/js?id=UA-110074814-1"></script>
	<script>
	  window.dataLayer = window.dataLayer || [];
	  function gtag(){dataLayer.push(arguments);}
	  gtag('js', new Date());

	  gtag('config', 'UA-110074814-1');
	</script>
</head>
...
<body class="<?=($bIndexBot ? "wbot" : "");?> site_<?=SITE_ID?> <?=($bIncludedModule ? "fill_bg_".strtolower(CNext::GetFrontParametrValue("SHOW_BG_BLOCK")) : "");?>" id="main">
	<div id="panel" style="position: relative;z-index: 999;"><?$APPLICATION->ShowPanel();?></div>
	<?if(!$bIncludedModule):?>
