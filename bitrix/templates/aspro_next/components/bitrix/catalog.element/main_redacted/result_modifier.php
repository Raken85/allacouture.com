		...
		if($arOffer['MORE_PHOTO']){
			foreach($arOffer['MORE_PHOTO'] as $i => $arImage){
				if($arImage["ID"]){
					...
					$arOffer['MORE_PHOTO'][$i]["SMALL"] = CFile::ResizeImageGet($arImage["ID"], array("width" => 500, "height" => 700), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
					$arOffer['MORE_PHOTO'][$i]["THUMB"] = CFile::ResizeImageGet($arImage["ID"], array("width" => 102, "height" => 102), BX_RESIZE_IMAGE_PROPORTIONAL, true, array());
				}
			}
		}
...
//to form template title/description meta.
$cp = $this->__component;
if (is_object($cp))
{
	$cp->arResult['PREVIEW_TO_DESC'] = $arResult["PREVIEW_TEXT"];
	$arResult['PREVIEW_TO_DESC'] = $arResult["PREVIEW_TEXT"];
	$cp->SetResultCacheKeys(array('PREVIEW_TO_DESC'));
}
?>
