	...
	<!--noindex-->
		<div class="sort_filter">
			...
			<?foreach($arAvailableSort as $key => $val):?>
				...
				<span class="hidden-link-class" data-link="<?=$url;?>" data-class="sort_btn <?=($sort == $key ? 'current' : '')?> <?=$sort_order?> <?=$key?>" rel="nofollow">
					<i class="icon" title="<?=GetMessage('SECT_SORT_'.$key)?>"></i><span><?=GetMessage('SECT_SORT_'.$key)?></span><i class="arr icons_fa"></i>
				</span>
			<?endforeach;?>
			<?
		...
		<div class="sort_display hidden-xs">
			<?foreach($arDisplays as $displayType):?>
				...
				<span class="hidden-link-class" data-link="<?=$url;?>" data-class="sort_btn <?=$displayType?> <?=($display == $displayType ? 'current' : '')?>"><i title="<?=GetMessage("SECT_DISPLAY_".strtoupper($displayType))?>"></i></span>
			<?endforeach;?>
		</div>
