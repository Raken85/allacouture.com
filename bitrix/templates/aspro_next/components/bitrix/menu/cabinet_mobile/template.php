...
<div class="menu middle">
	<ul>
		<li<?=(CNext::isPersonalPage() ? ' class="selected"' : '')?>>
			...
			<?/*
			<a rel="nofollow" class="dark-color<?=($bParent ? ' parent' : '')?> 11111111111" href="<?=$link;?>">
			...
			*/?>
			<span class="hidden-link-class 11111111111" rel="nofollow" data-class="dark-color<?=($bParent ? ' parent' : '')?>" data-link="<?=$link;?>">
				<?=CNext::showIconSvg("cabinet", SITE_TEMPLATE_PATH."/images/svg/".($USER->IsAuthorized() ? 'User' : 'Lock')."_black.svg");?>
				<span><?=GetMessage('CABINET_LINK2')?></span>
				<?if($bParent):?>
					<span class="arrow "><i class="svg svg_triangle_right"></i></span>
