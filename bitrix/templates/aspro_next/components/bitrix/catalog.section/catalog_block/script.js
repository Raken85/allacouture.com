...
$(document).ready(function() {
  if( window.innerWidth <= 767 ){
    $('.tags__more').on('click', function(event) {
          event.preventDefault();
          event.stopPropagation();
          
          var button = $(this),
              block = $('.tags__block_more[data-block=' + button.data('block') + ']');
          
          button.toggleClass('active');
          
          if (button.hasClass('active')) {
            $('.tags__text', button).text('СВЕРНУТЬ');
            block.slideDown();
          } else {
            $('.tags__text', button).text('ПОКАЗАТЬ ЕЩЕ');
            block.slideUp();
          }
