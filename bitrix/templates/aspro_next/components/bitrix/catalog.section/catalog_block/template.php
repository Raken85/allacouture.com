						...
						<div>
							<div class="image_wrapper_block">
								...
								<a href="<?=$arItem["DETAIL_PAGE_URL"]?>" class="thumb shine" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>">
									<?
$URL = CFile::GetPath($arItem[PROPERTIES][MORE_PHOTO][VALUE][1]);
...
<?
									...
									<?if( !empty($arItem["PREVIEW_PICTURE"]) ):?>
										...
										<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>111" class="first"  src="<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>" alt="<?=$a_alt;?>" title="<?=$a_title;?>" />
										<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>222" class="second" src="<?=$URL?>" alt="<?=$a_alt;?>" title="<?=$a_title;?>" />
									<?elseif( !empty($arItem["DETAIL_PICTURE"])):?>
										<?$img = CFile::ResizeImageGet($arItem["DETAIL_PICTURE"], array( "width" => 170, "height" => 170 ), BX_RESIZE_IMAGE_PROPORTIONAL,true );?>
										<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>111" class="first" src="<?=$img["src"]?>" alt="<?=$a_alt;?>" title="<?=$a_title;?>" />
										<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>222" class="second" src="<?=$URL?>" alt="<?=$a_alt;?>" title="<?=$a_title;?>" />
									<?else:?>
										<?$img = CFile::ResizeImageGet($arItem["DETAIL_PICTURE"], array( "width" => 170, "height" => 170 ), BX_RESIZE_IMAGE_PROPORTIONAL,true );?>
										<img id="<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>111" class="first" src="<?=$img["src"]?>" alt="<?=$a_alt;?>" title="<?=$a_title;?>" />
...
<style>
#<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>222{
opacity:0;}
#<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>:hover img.first{
opacity:0;
}
#<? echo $arItemIDs["ALL_ITEM_IDS"]['PICT']; ?>:hover img.second{
opacity:1;
}
</style>
...
<? if ( $arResult['~UF_TAGS'] ) { ?>
  <div class="tags">
    <div class="tags__content">
      <div class="tags__title">Вам может понравиться:</div>
      <div class="tags__block">
        <? $k = 0; 
        foreach( $arResult['~UF_TAGS'] as $arTag ) { 
        $k++; ?>
          <?=$arTag ?>
          <? if ( $k == 10 ) { ?>
            <span class="tags__block_more" data-block="tags">
              <span class="tags__block">
            <? } ?>
          <? } ?>
          <? if ( count($arResult['~UF_TAGS']) > 10 ) { ?>
            </span>
          </span>
          <a href="#" class="tags__more" data-block="tags"><span class="tags__text">ПОКАЗАТЬ ЕЩЕ</span></span></a>
        <? } ?>
      </div>
    </div>
  </div>
