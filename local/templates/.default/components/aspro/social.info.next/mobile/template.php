...
<div class="social-icons">
	...
	<ul>
		...
		<?if(!empty($arResult['SOCIAL_GOOGLEPLUS'])):?>
			<li class="gplus">
				<a href="mailto:<?=$arResult['SOCIAL_GOOGLEPLUS']?>" class="dark-color" target="_blank" rel="nofollow" title="<?=GetMessage('TEMPL_SOCIAL_GOOGLEPLUS')?>">
					<?=CNext::showIconSvg("gp", SITE_TEMPLATE_PATH."/images/svg/social/Mailru.svg");//Googleplus?>
		...
		<?if(!empty($arResult['SOCIAL_MAIL'])):?>
			<li class="mail">
				<a href="<?=$arResult['SOCIAL_MAIL']?>" class="dark-color" target="_blank" rel="nofollow" title="<?=GetMessage('TEMPL_SOCIAL_MAILRU')?>">
					<?=CNext::showIconSvg("ml social_whatsapp_custom", SITE_TEMPLATE_PATH."/images/svg/social/Whatsapp_black.svg");?>
