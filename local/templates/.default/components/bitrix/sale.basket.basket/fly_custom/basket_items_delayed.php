		...
		<thead>
			<tr>
				...
				<?foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
					...
					elseif ($arHeader["id"] == "QUANTITY"){ continue;}
					elseif ($arHeader["id"] == "SUMM"){ continue;}
					if ($arHeader["id"] == "NAME"):
					?>
				...
				<tr data-id="<?=$arItem["ID"]?>" data-iblockid="<?=$arItem["IBLOCK_ID"]?>" product-id="<?=$arItem["PRODUCT_ID"]?>">
					<? foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
						...
						if ($arHeader["id"] == "NAME"):?>
							...
							<td class="name-cell">
                                <span><?=$arItem['SECTION_NAME']?></span><br>
								<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?><a class="fly_basket_name" href="<?=$arItem["DETAIL_PAGE_URL"]?>"><?endif;?><?=$arItem["NAME"]?><?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?></a><?endif;?><br />
								<?if ($bPropsColumn):?>	
						...
						<?elseif ($arHeader["id"] == "PRICE"):?>
							<td class="cost-cell <?=( $bTypeColumn ? 'notes' : '' );?>">
								<div class="cost prices clearfix">
									<?if (false && strlen($arItem["NOTES"]) > 0 && $bTypeColumn):?>
